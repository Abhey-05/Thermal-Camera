...
float xRatios[240];
float yRatios[320];
float xOppositeRatios[240];
float yOppositeRatios[320];
...
if (interpolationMode == 0) {
  ...
  for (int y = 0; y < 320; y++) { // Now 320 pixels tall
    int yIndex = (y / 13) * 32; // Scale 24 rows to 320 pixels (320 / 24 ≈ 13.33)
    for (int x = 0; x < 240; x++) { // Now 240 pixels wide
      float val = frame[yIndex + (x / 7)]; // Scale 32 columns to 240 pixels (240 / 32 ≈ 7.5)
      drawPixel(239 - x, y, val); // Adjust for new width
    }
  }
} else if (interpolationMode == 1) {
  for (int y = 0; y < 320; y++) {
    int yIndex = (y / 13) * 32;
    int yNextIndex = ((y / 13) + 1) * 32;
    for (int x = 0; x < 240; x++) {
      int xIndex = x / 7;
      float val = (frame[yIndex + xIndex] + frame[yIndex + xIndex + 1] + frame[yNextIndex + xIndex] + frame[yNextIndex + xIndex + 1]) / 4.0;
      drawPixel(239 - x, y, val);
    }
  }
} else if (interpolationMode == 2) {
  for (int y = 0; y < 320; y++) {
    int yIndex = (y / 13) * 32;
    float y_ratio = (y % 13) / 13.0;
    float y_opposite_ratio = 1 - y_ratio;
    for (int x = 0; x < 240; x++) {
      float x_ratio = (x % 7) / 7.0;
      float x_opposite_ratio = 1 - x_ratio;
      int x_over_7 = x / 7;
      float val = y_opposite_ratio * (x_opposite_ratio * frame[yIndex + x_over_7] + x_ratio * frame[yIndex + x_over_7 + 1]) + y_ratio * (x_opposite_ratio * frame[(yIndex + 32) + x_over_7] + x_ratio * frame[(yIndex + 32) + x_over_7 + 1]);
      drawPixel(239 - x, y, val);
    }
  }
} else if (interpolationMode == 3) {
  int yIndex, xIndex, yNextIndex;
  for (int y = 0; y < 320; y++) {
    yIndex = (y / 13) * 32;
  ...
  sprite.drawString("T Min: " + String(tempMinRead) + " C", 15, 290); // Adjusted for 320 height
sprite.drawString("T Max: " + String(tempMaxRead) + " C", 140, 290);
sprite.drawString("Tc: " + String(tempCenter, 1) + "C", 75, 290);
sprite.drawLine(115, 160, 125, 160, TFT_WHITE); // Adjusted crosshair
sprite.drawLine(120, 155, 120, 165, TFT_WHITE);
  ...
  if (Menu == 1) {
  MenuChange = false;
  sprite.fillRect(20, 80, 200, 160, TFT_BLACK); // Adjusted for new dimensions
  sprite.fillRect(20, 80, 200, 10, TFT_WHITE);
  sprite.setTextFont(0);
  sprite.setTextSize(1);
  sprite.setTextColor(TFT_BLACK);
  sprite.drawString("Settings", 100, 81);
  sprite.setTextColor(TFT_WHITE);
  for (int i = 0; i < 7; i++) {
    if (i == Menuitem) sprite.setTextColor(TFT_GREEN);
    int y = 95 + (15 * i);
    sprite.drawString(menuItems[i], 25, y);
    if (i == 0) sprite.drawString(ASindex[AutoScale], 120, y);
    else if (i == 1) sprite.drawString(String(MinT) + " C", 120, y);
    else if (i == 2) sprite.drawString(String(MaxT) + " C", 120, y);
    else if (i == 3) sprite.drawString(IPindex[interpolationMode], 120, y);
    else if (i == 4) sprite.drawString("Palette " + String(paletteIndex), 120, y);
    else if (i == 5) sprite.drawString(RRindex[RefreshRate], 120, y);
    else if (i == 6) sprite.drawString(String(BLPWM) + " %", 120, y);
    sprite.setTextColor(TFT_WHITE);
  }
}
...
void GIFDraw1(GIFDRAW *pDraw) {
  ...
  if (pDraw->ucHasTransparency) {
    ...
    while (x < iWidth) {
      ...
      if (d > &usTemp[0]) {
        sprite.pushImage(pDraw->iX + x +40, y + 80, d - &usTemp[0], 1, usTemp);  // Push the image to the sprite
        x += d - &usTemp[0];
      }
      c = ucTransparent;
      while (c == ucTransparent && s < pEnd) {
    ...
    sprite.pushImage(pDraw->iX + 40 , y + 80 , iWidth, 1, usTemp);  // Push the image to the sprite
  }
}
...
void setup() {
 ...
 sprite.createSprite(TFT_WIDTH, TFT_HEIGHT); // 240x320
